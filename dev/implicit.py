...
def implicit_draft(example: int = 0, no_show: bool = False, step: str = '13i'):
    ...
    from references import ddy0, ddy1
    from references import jac0, jac1, jac2, jac3, jac4
    from pade import pade_31, pade_32, pade_32, pade_40, pade_41, pade_42
    from poly import poly_13i, poly_33i, poly_43i, poly_44i, jac_43i, jac_13i
    from matplotlib import pyplot as plt

    examples = ((diffs0, ddy0, jac0, -0.5, 0., 0.5),
            (diffs1, ddy1, jac1, -0.0185, 0., 0.0185),
            (diffs2, jac2, -0.03, 0., 0.06),
            (diffs3, jac3, -0.0275, 0., 0.055),
            (diffs4, jac4, -0.00055, 0., 0.0011),)
    ...
    for iteration in range(10):
        ...
        y_calc, dy_calc = f_step(F0, (y, dy, ddy), Dx)
        print('y, dy calc', y_calc, dy_calc)

        err = np.array((y - y_calc, dy - dy_calc))
        if not no_show:
            axs[2].plot(iteration, err[0], '.', color = 'black')
