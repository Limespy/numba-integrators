...
class Test_convert():
    ...
    def test_correct(self):
        assert np_array_tuple_compare(_API.convert(self.y0,
                                                   self.rtol,
                                                   self.atol),
                                      (self.y0, self.rtol, self.atol))
    # ------------------------------------------------------------------
    def test_tol_fill(self):
        assert np_array_tuple_compare(_API.convert(self.y0, 1., 2.),
                                      (self.y0, self.rtol, self.atol))
    # ------------------------------------------------------------------
    def test_y0_from_list(self):
        assert np_array_tuple_compare(_API.convert(list(self.y0),
                                                   self.rtol,
                                                   self.atol),
                                      (self.y0, self.rtol, self.atol))
    # ------------------------------------------------------------------
    def test_single_value_y0(self):
        assert np_array_tuple_compare(_API.convert(1, 2, 3),
                                      (np.array((1,), dtype = np.float64),
                                       np.array((2,), dtype = np.float64),
                                       np.array((3,), dtype = np.float64)))
    # ------------------------------------------------------------------
    def test_dtype_change(self):
        assert np_array_tuple_compare(_API.convert(self.y0.astype(np.int32),
