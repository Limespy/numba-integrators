...
if TYPE_CHECKING:
    ...
    Callable = Iterable = Sequence = tuple
    Any = npA = npAFloat64 = npAInt64 = ODEA_return = object
    Arrayable = nbType = nbSignature = object
# ======================================================================
...
class RKSolverBase(Solver):
    ...
    def step(self) -> bool:
        ...
        return False
    # ------------------------------------------------------------------
    def reboot(self, first_step: np.float64 = np.float64(0.)):
        y_len = len(self.y)
        self._init_K(y_len)
        self._len = 1./y_len
        self.step_size = 0.
        ...
        h_abs, self._nfev = ((np.abs(first_step), np.int64(1))
                              if first_step else
                              (self._select_initial_step(h_end), np.int64(2)))
...
if IS_NUMBA:
    ...
    def jitclass_from_dict(*args, **kwargs): # type: ignore[misc]
        return lambda x: x
